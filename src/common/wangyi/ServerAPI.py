...
class ServerAPI():
    ...
    def checkSumBuilder(self):
        ...
        self.CurTime = int(
            time.time());  # 当前UTC时间戳，从1970年1月1日0点0 分0 秒开始到现在的秒数(String)
        join_string = self.AppSecret + self.Nonce + str(self.CurTime);

        self.CheckSum = hashlib.sha1(
            join_string).hexdigest();  # SHA1(AppSecret + Nonce + CurTime),三个参数拼接的字符串，进行SHA1哈希计算，转化成16进制字符(String，小写)
    ...
    def updateUinfo(self, accid, name='', icon='', sign='', email='', birth='',
                    mobile='', gender='0', ex=''):
    ...
    def sendMsg(self, fromx, ope, to, typex, body, option=dict(
        {"push": 'false', "roam": 'true', "history": 'false',
         "sendersync": 'true', "route": 'false'}), pushcontent=''):
    ...
    def sendAttachMsg(self, fromx, msgtype, to, attach, pushcontent='',
                      payload=dict({}), sound=''):
    ...
    def createGroup(self, tname, owner, members, announcement='', intro='',
                    msg='', magree='0', joinmode='0', custom='0'):
    ...
    def updateGroup(self, tid, owner, tname, announcement='', intro='',
                    joinmode='0', custom=''):
    ...
    def querySessionMsg(self, fromx, to, begintime, endtime='', limit='100',
                        reverse='1'):
    ...
    def queryGroupMsg(self, tid, accid, begintime, endtime='', limit='100',
                      reverse='1'):
